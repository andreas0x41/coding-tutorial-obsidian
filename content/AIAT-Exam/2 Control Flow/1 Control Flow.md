...
- **Selection**: [[AIAT-Exam/2 Control Flow/2 Conditions|Conditional statements]] like`if-else` allow decisions based on conditions.
- **Iteration**: Loops like [[AIAT-Exam/2 Control Flow/3 While Loop|while]] or [[AIAT-Exam/2 Control Flow/4 For Loop|for]] repeat code while a condition is true or for a number of iterations.
- **Jumps**: `break` exits loops, `continue` skips to the next iteration
- **Subroutines**: [[AIAT-Exam/2 Control Flow/5 Function|Functions]] encapsulate reusable code, called with arguments and return values.
...
Selection allows the program to make decisions. The program chooses what code to execute next based on a condition. The basis for the decisions is **[[AIAT-Exam/2 Control Flow/2 Conditions|if]]** a [[AIAT-Exam/1 General/5 Datatype#Boolean|boolean]] is `True`, or whether a pattern matches.
...
This allows you to repeat a block of code multiple times. The block is repeatedly executed **[[AIAT-Exam/2 Control Flow/3 While Loop|while]]** a [[AIAT-Exam/1 General/5 Datatype#Boolean|boolean]] condition is true or **[[AIAT-Exam/2 Control Flow/4 For Loop|for]]** a fixed number of iterations. 
...
# Jumps
Jumps allow the program to exit the normal flow of execution and skip part of the code or go to a completely different position. Python allows you to exit a loop using **[[AIAT-Exam/2 Control Flow/3 While Loop#Break|break]]** or go to the next iteration of a loop using **[[AIAT-Exam/2 Control Flow/3 While Loop#Continue|continue]]**.
