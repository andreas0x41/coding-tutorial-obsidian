...
# Deleting
You can use the [[content/Theory Exam/1 General/8 Keyword|keyword]] `del` to delete a variable/the object reference it stores. If you are working with a [[content/Theory Exam/1 General/5 Datatype#Mutable|mutable]] collection of type list or dictionary you can also use indexing and slicing to remove individual items.
...
If you do a similar thing with an object that is [[content/Theory Exam/1 General/5 Datatype#Mutable|mutable]], you can directly change its value in place. This means that the actual value of the object is changed. Since both of the variables are pointing to the same object, **both** of their values will change.
