...
# Basic
A `while` loop is similar to a [[content/Theory Exam/Temp/2 Control Flow/2 Conditions|if statement]] and also has a [[content/Python/1 General/5 Datatype#Boolean|boolean]] condition. But instead of executing the code block inside once, it is executed repeatedly as long as the condition is [[content/Python/1 General/5 Datatype#Truthy and Falsy|truthy]].
