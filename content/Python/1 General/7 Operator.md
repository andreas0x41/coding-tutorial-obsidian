...
- **Logical Operators** chain conditions together	
	-  **`and`**: [[Python/1 General/5 Datatype#Truthy and Falsy|Truthy]] if both conditions are Truthy.
	-  **`or`**: [[Python/1 General/5 Datatype#Truthy and Falsy|Truthy]] if at least one condition is Truthy.
	- **`not`**: Inverts the boolean value.
- Operators are evaluated based on precedence, e.g. `()`, `**`, `*` `/`, `+` `-`, comparison, logical, assignment

# Assignment Operator
The assignment operator `=` is used to [[Python/1 General/6 Variable#Creating/Assigning|assign]] a value to a [[Python/1 General/6 Variable|variable]] .
...
## Result Datatype
The [[Python/1 General/5 Datatype|datatype]] of the result of an operation only depends on the datatypes or the two operands. 

Addition `+`, Subtraction `-`, Multiplication `*`, Floor Division `//`, Modulus `%` and Exponentiation `**` will return an integer if both of the operands are [[Python/1 General/5 Datatype#Integer|integers]]. If one of them is [[Python/1 General/5 Datatype#Complex|complex]] the result is complex. Otherwise, if one of them is a [[Python/1 General/5 Datatype#Float|float]], the result will be float. 

The true Division `/` will always return a [[Python/1 General/5 Datatype#Float|float]]. If one of the operands is [[Python/1 General/5 Datatype#Complex|complex]], it will return complex. 
...
## Collection Usage
The operators `+` and `*` can also be used with [[Python/1 General/5 Datatype#Collection Datatypes|sequence datatypes]]. The `+` operator is used to concatenate / combine two strings, lists, or tuples of the **same** datatype.
...
Relational Operators always compare two values and return a [[Python/1 General/5 Datatype#Boolean|boolean]].
...
[[Python/1 General/5 Datatype#Indexing|Indexing]] and [[Python/1 General/5 Datatype#Slicing|Slicing]] are explained in detail on the datatypes page.

## Membership Checking
The operator `in` is used to check if a value/element exists in a collection. It always returns a [[Python/1 General/5 Datatype#Boolean|boolean]] and uses the syntax `value in collection`. So basically it checks if any of the elements in the collection is [[#(In-)Equality|equal]] to the value.
...
This operator can be used with a [[Python/1 General/5 Datatype#Dictionary|dictionary]] to check if a specific **key** exists in the dictionary. If you want to check if a value exists in a dictionary you have to use the built-in method `.values()`.
...
When using this operator with a [[Python/1 General/5 Datatype#String|string]] it checks if the value is a substring of the whole string.
...
| `and`                            | Logical AND                                           |
| `or`                             | Logical OR                                            |
| `if` ... `else`                  | [[Python/2 Control Flow/2 Conditions#Ternary Operator\|Ternary Operator]] |
