...
A datatype defines how different types of information/data are stored and behave. Examples of the behavior can be found on the pages [[content/Python/1 General/7 Operator|Operators]], [[content/Python/2 Control Flow/6 Built-in Functions & Methods|Built-in Functions & Methods]], ...
...
| [[content/Python/1 General/5 Datatype#Tuple\|Tuple]]           | tuple()   | `(1, 1, 2, 3`)                | [[#Mutable\|unmutable]] collection of any datatype                                               |
| [[content/Python/1 General/5 Datatype#Set\|Set]]               | set()     | `{2, 1, 3}`                   | [[#Mutable\|mutable]] collection of any datatype without order, indexing or duplicates           |
| [[content/Python/1 General/5 Datatype#Dictionary\|Dictionary]] | dict()    | `{"a": 1, "c": "5"}`          | 1 to 1 mapping of not [[#Mutable\|mutable]] keys to [[#Mutable\|mutable]] values of any datatype |
| [[content/Python/1 General/5 Datatype#None\|None]]             |           | `None`                        | None means that there is no value                                                                |
| [[content/Python/1 General/5 Datatype#Complex\|Complex]]       | complex() | `1j`, `1.2-0.5j`              | complex number with real and imaginary part                                                      |
| [[content/Python/1 General/5 Datatype#Range\|Range]]           | range()   | `range(8)`, `range(2, 99, 3)` | range of numbers                                                                                 |
...
The datatype Boolean can hold a truth value. There are only two possible options for this value True or False (can also be thought of as 1/0 Yes/No On/Off). This datatype is very important for Control Flow like [[content/Python/2 Control Flow/2 Conditions|Conditions]] because it is usually based on many "Yes/No Questions", which are answered with a Boolean value. You can use the function `bool()` to convert a value to a Boolean, or create one with value `False` if no argument is given.
...
| ------------------------------------- | ------- | ---------------------- | -------- | ------- | --------------------------------------------------- |
| [[content/Python/1 General/5 Datatype#String\|String]]         | No      | Yes                    | Yes      | Yes     | Text processing and manipulation                    |
| [[content/Python/1 General/5 Datatype#List\|List]]             | Yes     | Yes                    | Yes      | Yes     | Storing and manipulating mutable sequences          |
| [[content/Python/1 General/5 Datatype#Tuple\|Tuple]]           | No      | Yes                    | Yes      | Yes     | Fixed/Constant collections of items                 |
| [[content/Python/1 General/5 Datatype#Set\|Set]]               | Yes     | Yes (but no order)     | No       | No      | Ensuring unique elements in a collection, Set Logic |
| [[content/Python/1 General/5 Datatype#Dictionary\|Dictionary]] | Yes     | keys, values, or pairs | Keys     | No      | Key-value pair storage and lookups                  |
| [[content/Python/1 General/5 Datatype#Range\|Range]]           | No      | Yes                    | Yes      | Yes     | Generating sequences of numbers                     |
| [[content/Python/1 General/5 Datatype#Generator\|Generator]]   | No      | Yes                    | No       | No      | Efficient iteration over large datasets             |

### Mutable
A mutable object can be modified after it is created, a immutable object can not. This means you can change, add, or remove parts or all of its content without creating a new [[content/Python/1 General/6 Variable#Memory Objects|object]]. For example, you can change the value of the second item in a list, but you can not change the value of the second item in a tuple or string. All basic datatypes are immutable. So when it seems like the value changes, it is actually a completely new [[content/Python/1 General/6 Variable#Memory Objects|object]]. You can find the difference in behavior between mutable and immutable variables [[content/Python/1 General/6 Variable#Mutability Behaviour|here]].
...
A [[content/Python/1 General/5 Datatype#Dictionary|dictionary]] does not have classic ordered indexing with numbers. Instead, each element is a combination of key and value, and you use the key instead of an index
### Slicing
...
You can use the function `dict()` to convert values to a dictionary or create an empty dictionary `{}` if no arguments are given. You can use `dict()` with no [[content/Python/2 Control Flow/5 Function#Parameters|arguments]], an [[#Iterable]] of key and value pairs, or keyword arguments.
...
## Truthy and Falsy
In Python, any value can be evaluated (e.g. in a [[content/Python/1 General/7 Operator#Logical Operators|logical operator]], [[content/Python/2 Control Flow/2 Conditions|condition]], ...) like a boolean. Python will treat it as truthy (`True`) or falsy (`False`) based on its type and value.
