...
Using a variable as an argument will create a new pointer to the same [[AIAT/1 General/6 Variable#Memory Objects|memory object]]. So you have to be careful with the [[AIAT/1 General/6 Variable#Mutability Behaviour|mutability behavior]]. 
...
You can see that changing the value of [[AIAT/1 General/6 Variable#Mutability Behaviour|mutable variables]] applies to the [[AIAT/1 General/6 Variable#Memory Objects|memory object]] itself and therefore also outside the function.
# Return
...
The function execution ends as soon as it hits a `return`, everything after that will not be executed. You can see in the following code example, that the [[AIAT/2 Control Flow/2 Conditions#Else|else]] keywords are not necessary here, because the execution will only continue if the condition before was `False`.
...
If a function does not return anything, it returns [[AIAT/1 General/5 Datatype#None|None]] by default.
...
You can return multiple comma-separated values, which will be interpreted like a [[AIAT/1 General/5 Datatype#Tuple|tuple]]. You can extract the individual values using [[AIAT/1 General/5 Datatype#Indexing|indexing]] or [[AIAT/1 General/7 Operator#Multiple Value Assignment|multiple value assignment]].
...
# Scope
The general principle of scope is explained on the [[AIAT/1 General/6 Variable#Scope|variables]] page. You can refer to a global variable from inside a function using the `global`[[AIAT/1 General/8 Keyword|keyword]].
...
You can refer to a variable one layer outside of the current function using the `nonlocal`[[AIAT/1 General/8 Keyword|keyword]].
