...
# Syntax
Python's syntax is minimal and highly readable. In general Python code will run line by line from top to bottom. More complex behavior is possible using [[content/AIAT/2 Control Flow/1 Control Flow|Control Flow]]. 
- **`print()`** is used for [[content/AIAT/1 General/2 Output|output]], **`input()`** for [[content/AIAT/1 General/3 Input|input]], and **`#`** for line [[content/AIAT/1 General/4 Comment|comments]]
- Python supports all basic (mathematical) [[content/AIAT/1 General/7 Operator|Operators]] and many more
- [[content/AIAT/1 General/6 Variable|Variables]] automatically get a [[content/AIAT/1 General/5 Datatype|datatype]] that can change at any time
