...
# Syntax
Python's syntax is minimal and highly readable. In general Python code will run line by line from top to bottom. More complex behavior is possible using [[AIAT/2 Control Flow/1 Control Flow|Control Flow]]. 
- **`print()`** is used for [[AIAT/1 General/2 Output|output]], **`input()`** for [[AIAT/1 General/3 Input|input]], and **`#`** for line [[AIAT/1 General/4 Comment|comments]]
- Python supports all basic (mathematical) [[AIAT/1 General/7 Operator|Operators]] and many more
- [[AIAT/1 General/6 Variable|Variables]] automatically get a [[AIAT/1 General/5 Datatype|datatype]] that can change at any time
