...
Variables are used to store data. You can think of them like a container. The shape of the container is defined by the [[content/AIAT/1 General/5 Datatype|Datatype]] and then you can put any fitting value inside the container. Python variables are labels that point to an object in memory that stores a value.
...
- The [[content/AIAT/1 General/5 Datatype|(data-)type]] of the object that defines what kind of object it is (e.g., `int`, `str`, `list`). This is returned by the `type()` function.
...
You can check if two variables point to the same object in memory (have the same identifier) using the `is` [[content/AIAT/1 General/8 Keyword|keyword]].
# Creating/Assigning
...
# Deleting
You can use the [[content/AIAT/1 General/8 Keyword|keyword]] `del` to delete a variable/the object reference it stores. If you are working with a [[content/AIAT/1 General/5 Datatype#Mutable|mutable]] collection of type list or dictionary you can also use indexing and slicing to remove individual items.
...
3. **Cannot be a reserved [[content/AIAT/1 General/8 Keyword|keyword]]**: Avoid Python’s built-in keywords like `if`, `for`, `class`, `del`, ...
...
If you do a similar thing with an object that is [[content/AIAT/1 General/5 Datatype#Mutable|mutable]], you can directly change its value in place. This means that the actual value of the object is changed. Since both of the variables are pointing to the same object, **both** of their values will change.
...
You can find detailed examples and usage of the `global` and `nonlocal` [[content/AIAT/1 General/8 Keyword|keywords]] on the page about [[content/AIAT/2 Control Flow/5 Function#Variable Scope|functions]].
